...
class Seller:
    ...
    def get_review_score(self):
        ...
        """
        o_reviews = self.order.get_review_score()
        o_sellers = self.data['order_items'][['order_id', 'seller_id']].drop_duplicates()
        review_s = o_sellers.merge(o_reviews, on='order_id')
        review_s['score_cost'] = review_s.review_score.map({5:0,
                                                            4:0,
                                                            3:40,
                                                            2:50,
                                                            1:100})

        review_s = review_s.groupby('seller_id', as_index=False).agg({'dim_is_one_star':'mean',
                                                                    'dim_is_five_star':'mean',
                                                                    'review_score':'mean',
                                                                    'score_cost':'sum'})
        review_s.rename(columns = {"seller_id": "seller_id",
                             "dim_is_one_star": "share_of_one_stars",
                            "dim_is_five_star": "share_of_five_stars",
                                "review_score": "review_score",
                                    "score_cost": "review_cost"}, inplace = True)


        review_s = review_s[['seller_id', 'share_of_five_stars', 'share_of_one_stars', 'review_score', 'review_cost']]
        return review_s
